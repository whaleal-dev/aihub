...
Realtime 在 AIHub 当前是一条 **很薄但正式存在的长连接能力面**。  
...
- `com.whaleal.aihub.listener.RealtimeListener`
...
> AIHub 当前的 Realtime 能力是一条很薄的正式建连抽象：它统一了 OpenAI realtime 的 URL、鉴权头和 WebSocket 入口，但还没有把事件协议、失败转发、重连治理和会话恢复做成完整 runtime。用它时，应该把它理解成“长连接入口层”，而不是“完整 realtime 会话框架”。
