...
description: 总览模型请求在 AIHub 基座里如何被建模、投影、发送、流式消费和回读，厘清 Chat、Responses、Messages 三条主线的边界。
tags: [concept]
---

# Model Access 总览

`Model Access` 这一章讲的不是“AIHub 支持哪些模型名字”，而是 **模型请求在 AIHub 基座里如何被建模、投影、发送、流式消费和回读**。
...
它仍然是 `aihub/` 基座层的问题，不是 Agent 或 Coding Agent 的上层专属话题。

## 2. 这一章真正覆盖什么

这一章主要覆盖五个问题：

1. 为什么 AIHub 同时保留 `Chat` 与 `Responses`
...
第一次进入这一章，最重要的不是先看某个 provider，而是先分清 AIHub 内部有三条不同的模型访问主线，分别对应三族原生协议：
...
## 5. AIHub 如何处理“统一请求”与“provider 差异”

AIHub 的请求对象策略不是简单追求字段最少，而是把主语义固定下来，再把差异留给适配层。
...
这说明多模态在 AIHub 里是请求协议问题，不是外部能力接入问题。
...
> `Model Access` 在 AIHub 里讲的是“请求如何被建模并送进 provider”，不是“模型能做什么”。当前基座保留了两条清晰主线：`Chat` 负责更广覆盖的消息式访问，`Responses` 负责更结构化的事件式访问；二者共享统一工具与多模态基座，但并不应被看成同一接口的两种皮肤。
