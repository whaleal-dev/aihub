...
在 AIHub 里，“streaming” 不是一个统一的 token 输出概念，而是三条不同主线各自对应的消费模型：
...
从这组字段就能看出，AIHub 的 Chat 流式并不只是“token 来了就打印”，而是已经支持同时消费：
...
AIHub 在 listener 层已经把这件事吸收掉了，上层运行时不必重新自己拼装。
...
> AIHub 的 streaming 不是单一“token 流”抽象。`Chat` 流式围绕消息增量、finish reason 和 tool call 聚合组织；`Responses` 流式围绕 event type、response 生命周期和状态闭合组织；`Messages` 流式把 Anthropic 原生事件以类型化回调暴露，状态累积交给调用方。三者都走 SSE，但适合完全不同的上层消费方式。
