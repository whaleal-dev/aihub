...
`Messages` 是 AIHub 里与 `Chat`（OpenAI Chat Completions）、`Responses`（OpenAI Responses）并列的第三条模型访问主线，走的是 **Anthropic Messages 协议**（`POST /v1/messages`）。
...
:::tip 本页代码都是可跑通的
下面的示例来自
[`MessagesDocExamplesLiveTest`](https://github.com/whaleal-dev/aihub/blob/main/aihub/src/test/java/com/whaleal/aihub/docs/MessagesDocExamplesLiveTest.java)，
...
mvn -pl aihub test -Plive-provider-tests -Dtest=MessagesDocExamplesLiveTest
```
...
底层是 Anthropic Messages 的 HTTP 客户端，与 Chat / Responses 并列，不是 Agent 运行时。
...
`aihub-spring-boot-starter` 通过 `@ConfigurationProperties(prefix = "aihub.anthropic")` 暴露 Anthropic 配置，auto-config 把 `aihub.anthropic.*` 灌进 `AnthropicConfig`：
...
4. [模型接入总览](/docs/capabilities/models/overview)
