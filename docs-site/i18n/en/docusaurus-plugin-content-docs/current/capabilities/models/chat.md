---
title: Chat 主线
description: 解析 AIHub Chat 主线的请求对象、自动 tool loop、passThroughToolCalls、SseListener 流式聚合和多模态接入等运行时行为。
tags: [concept]
---

# Chat 主线
`Chat` 是 AIHub 当前最成熟、provider 覆盖最广、也最容易先跑通的一条模型访问主线。
...
[`ChatDocExamplesLiveTest`](https://github.com/whaleal-dev/aihub/blob/main/aihub/src/test/java/com/whaleal/aihub/docs/ChatDocExamplesLiveTest.java)，
...
mvn -pl aihub test -Plive-provider-tests -Dtest=ChatDocExamplesLiveTest
```
...
其中真正定义“Chat 在 AIHub 中是什么”的，不只是请求对象，还包括 `OpenAiChatService` 内部那条自动工具调用循环。
...
也就是说，AIHub 在 `Chat` 层已经把“本地能力注册”和“provider payload 组装”分开了。
...
这是理解 AIHub `Chat` 和普通 provider SDK 差异的关键。
...
这意味着 `Chat` 在 AIHub 中不是单次 RPC，而是一个可以本地闭环执行工具的对话循环。
...
这说明 Chat 流式在 AIHub 里已经是“可供运行时消费的聚合状态”，不是单纯 token 输出。
...
这意味着在 AIHub 里，多模态并不是 Chat 之外的独立特殊链路，而是消息内容编码方式的扩展。
...
> AIHub 的 `Chat` 不是薄薄一层请求封装，而是一条成熟的消息式运行链：请求前会解析工具注册，收到 `tool_calls` 时可以自动闭环执行，流式阶段又由 `SseListener` 聚合文本、reasoning 和工具参数。因此它既适合快速接入，也足以支撑中等复杂度的本地 tool runtime。
