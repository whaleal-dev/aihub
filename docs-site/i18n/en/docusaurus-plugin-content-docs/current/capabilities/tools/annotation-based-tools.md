...
# 注解式工具
AIHub 推荐注解式工具，不是因为“代码更短”，而是因为它把 Java 类型、字段说明和 provider tool schema 绑定到了同一条生成链上。
...
本质上，AIHub 是在用 Java 类型系统生成模型工具契约。
...
这也是为什么 AIHub 工具参数更适合扁平结构，而不是深嵌套 DTO。
...
AIHub 的注解式工具，不是“给类贴几个标签”，而是一条从 Java 类型到 provider tool schema 的生成链。
