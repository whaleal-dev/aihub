---
title: SPI HTTP 栈
description: 讲清 AIHub HTTP stack SPI：通过 ServiceLoader 把 Dispatcher 与 ConnectionPool 注入 starter 构造的统一 OkHttpClient，默认实现靠 META-INF/services 注册，丢失会导致启动失败，仅 starter 装配链自动生效。
tags: [concept]
---

# SPI HTTP 栈
这一页讲的是 AIHub 当前少数真正已经 SPI 化的扩展面之一：**底层 `OkHttp` 并发调度与连接池策略**。
...
- `aihub-spring-boot-starter/.../AiConfigAutoConfiguration.initOkHttp()`

默认实现之所以能生效，不是因为代码里写死了 fallback，而是因为：

- `aihub/src/main/resources/META-INF/services/com.whaleal.aihub.network.DispatcherProvider`
- `aihub/src/main/resources/META-INF/services/com.whaleal.aihub.network.ConnectionPoolProvider`
...
AIHub 把这两个点抽出来，目的不是给 demo 增加炫技选项，而是让底层网络治理有一个正式入口。
...
所以默认行为的本质是“沿用 OkHttp 自身默认策略”，而不是 AIHub 对并发和连接做了特别积极的调优。
...
> AIHub 的 HTTP stack 扩展是当前少数真正走 SPI 的部分。它通过 `ServiceLoader` 把 `Dispatcher` 与 `ConnectionPool` 注入 starter 构造出的统一 `OkHttpClient`，从而影响整个 SDK 的底层网络行为。这里的关键不是“能不能自定义”，而是要清楚它是全局策略入口、依赖 `META-INF/services` 打包完整性，而且只会在对应装配链上自动生效。
