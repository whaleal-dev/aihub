---
title: 服务扩展
description: 讲清 AIHub service extension：新增顶层能力契约会扩大整个 SDK 公共 API 面，必须同步 AiService、AiServiceRegistry 与 FreeAiService 兼容入口，AiServiceFactory 不是 service 插件总线，仅在现有契约无法承载时才值得新增。
tags: [concept]
---

# 服务扩展
`service extension` 解决的是：**AIHub 是否要新增一条新的顶层能力契约**。  
这比 model extension 更重，因为你不是在补某个平台，而是在扩大整个 SDK 的公共能力面。

## 1. 当前已经存在的顶层能力面

在 `aihub/src/main/java/com/whaleal/aihub/service/` 下，当前已经存在这些正式接口：
...
只要新增顶层 service，你就在扩大 `aihub` 模块对外承诺的接口面。这会影响：
...
> 在 AIHub 里，service extension 改的是 SDK 的正式能力版图，而不是某个平台的局部实现。只有当现有顶层契约已经无法自然承载新语义时，才值得新增一条 service 面；否则更稳的做法是把变化吸收在现有 service 契约和 provider 适配层内部。
