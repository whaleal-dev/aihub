---
title: 模型扩展
description: 讲清 AIHub model extension：在不新增 PlatformType 的前提下，把新模型能力吸收进现有 provider 与现有契约，主战场是请求对象和 provider 适配层，强调把 provider 差异收敛在 provider service 内部而非泄漏到业务层。
tags: [concept]
---
...
AIHub 当前不同 service 面的 provider 覆盖并不对称。
...
> 在 AIHub 里，model extension 的核心不是“能不能换一个 model 字符串”，而是现有 provider 与现有顶层契约能否继续承载这项变化。只要平台边界和能力语义没变，最稳的做法就是把差异收敛在请求对象和 provider 适配层内部，而不是过早抬升到 provider 或 service 级扩展。
