...
# 请求与返回约定
这一页统一解释：**在 AIHub 里，请求应该怎么构造，返回应该怎么读，哪些字段是 SDK 本地语义，哪些字段才会真正进入 provider payload。**

很多接入问题看起来像 provider 不稳定，实际上只是没有建立统一的请求/返回约定。

## 1. AIHub 的基本策略是什么

AIHub 当前在模型访问层采用的是一种很明确的策略：
...
以 `ChatCompletion` 为例，下面这些字段在 AIHub 中很重要，但并不意味着它们会原样发送给 provider：
...
这说明 AIHub 并不是“请求对象怎么长，provider 就收什么”，而是在 service 层有一层正式的投影。
...
> AIHub 在请求/返回层的核心约定是：统一实体承载主语义，本地辅助字段承载注册与运行时语义，provider service 负责把它们投影成最终 payload，而 listener/response entity 负责把返回重新聚合成稳定读取面。理解这层约定，才能避免把 provider 差异和本地运行时语义混在一起。
