...
> `ORCAROUTER` 是 OpenAI 兼容聚合网关的命名入口：默认 host 为 `https://api.orcarouter.ai/`，Chat / Responses / Embedding / Audio / Image 复用 `OpenAi*Service`。一般中转平台仍用 `OPENAI` + 改 `apiHost`。  
> `GROK` 默认 host 为 `https://api.x.ai/`，Chat / Embedding / Image 复用 OpenAI 栈，Video 仍走 Grok 方言。  
> `GEMINI` 默认打官方 OpenAI 兼容层 `https://generativelanguage.googleapis.com/v1beta/openai/`，Chat / Embedding / Image 复用 OpenAI 栈。
...
- OpenAI
- OrcaRouter（复用 OpenAI Responses 栈）
- Doubao
- DashScope

这意味着 `Responses` 在 AIHub 里不是“每个平台都应当自动拥有”的替代接口，而是一条更聚焦的结构化访问主线。
...
- `Embedding` 只支持 OpenAI / OrcaRouter / Ollama
- `Audio` 只支持 OpenAI / OrcaRouter
- `Realtime` 只支持 OpenAI
...
> AIHub 当前的统一，不在于所有平台能力完全对称，而在于它用统一入口显式维护了一张不对称但清晰的 service 矩阵。理解这张矩阵，比记住某个 provider 的示例更重要，因为它直接决定了你能否正确设计多平台、多 service 的工程边界。
