...
> `Chat` 和 `Responses` 在 AIHub 中不是新旧关系，而是两条不同的模型访问主线。`Chat` 以消息和自动 tool loop 为中心，更适合广覆盖和快速接入；`Responses` 以事件和结构化 response 为中心，更适合 runtime、trace 和复杂交互。真正的选型标准不是“哪个更先进”，而是你要消费的是消息结果，还是过程状态。
