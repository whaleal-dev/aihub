...
:::tip 本页代码可跑通
非流式生成示例来自
[`ImageGenerationLiveTest`](https://github.com/whaleal-dev/aihub/blob/main/aihub/src/test/java/com/whaleal/aihub/docs/ImageGenerationLiveTest.java)，
已针对真实 OpenAI 兼容网关（grok-imagine-image）跑通。

本地复跑：`OPENAI_API_KEY=... OPENAI_API_HOST=... OPENAI_IMAGE_MODEL=... mvn -pl aihub test -Plive-provider-tests -Dtest=ImageGenerationLiveTest`，无 key 自动跳过。
