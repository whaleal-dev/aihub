...
description: Spring Boot 接入 AIHub 的最短成功路径：引入 starter、配置 aihub.*、注入 AiService 并发出第一个 ChatCompletion。
tags: [how-to]
---
...
1. 引入 `aihub-spring-boot-starter`
2. 在 `application.yml` 里配置 `aihub.*`
3. 注入 `AiService`
