package com.whaleal.aihub.platform.openai.video;
...
public class OpenAiVideoServiceTest {
    ...
    public void test_create_video_posts_json_to_videos_endpoint() throws Exception {
        ...
        try {
            ...
            Assert.assertEquals("veo3.1", body.path("model").asText());
            Assert.assertEquals("飞上天", body.path("prompt").asText());
            Assert.assertEquals(8, body.path("seconds").asInt());
            Assert.assertEquals("1280x720", body.path("size").asText());
            Assert.assertEquals(Boolean.TRUE, body.path("enable_upsample").asBoolean());
        } finally {
            server.shutdown();
        }
    }
    ...
    public void test_remix_posts_prompt_json() throws Exception {
        ...
        try {
            ...
            JsonNode body = Jsons.readTree(request.getBody().readUtf8());
            Assert.assertEquals("让背景变成蓝天", body.path("prompt").asText());
        } finally {
            server.shutdown();
        }
    }
