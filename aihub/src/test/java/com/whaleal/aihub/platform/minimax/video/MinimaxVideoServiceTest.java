package com.whaleal.aihub.platform.minimax.video;
...
public class MinimaxVideoServiceTest {
    ...
    public void test_create_posts_minimax_dialect_body() throws Exception {
        ...
        try {
            ...
            Assert.assertEquals("MiniMax-Hailuo-2.3", body.path("model").asText());
            Assert.assertEquals("一只猫在月光下奔跑", body.path("prompt").asText());
            Assert.assertEquals("https://cdn.example/first.png", body.path("first_frame_image").asText());
            Assert.assertEquals(Boolean.FALSE, body.path("promptenhance").asBoolean());
        } finally {
            server.shutdown();
        }
    }
    ...
    public void test_create_merges_extra_fields_for_vendor_params() throws Exception {
        ...
        try {
            ...
            Assert.assertEquals(Boolean.TRUE, body.path("promptenhance").asBoolean());
