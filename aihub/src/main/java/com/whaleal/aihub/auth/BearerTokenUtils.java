package com.whaleal.aihub.auth;
...
public class BearerTokenUtils {
    ...
    private static final long TOKEN_TTL_MILLIS = EXPIRE_MILLIS - (60 * 1000L);
    private static final ConcurrentHashMap<String, CachedToken> CACHE = new ConcurrentHashMap<String, CachedToken>();
    ...
    public static String getToken(String apiKey) {
        ...
        // 缓存Token
        String token = getCachedToken(apiKey);
        if (token != null) {
            return token;
        }
        // 创建Token
        ...
        CACHE.put(apiKey, new CachedToken(token, System.currentTimeMillis() + TOKEN_TTL_MILLIS));
        return token;
    }
    ...
    private static String getCachedToken(String apiKey) {
        CachedToken cached = CACHE.get(apiKey);
        if (cached == null) {
            return null;
        }
