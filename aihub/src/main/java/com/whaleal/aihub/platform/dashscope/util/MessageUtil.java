package com.whaleal.aihub.platform.dashscope.util;
...
/**
 * DashScope Message 与统一 ChatMessage 互转。
 *
 * @author 恒哥
 */
public class MessageUtil {
    ...
    public static ChatMessage convert(Message message) {
        ChatMessage chatMessage = BeanCopy.copy(message, ChatMessage.class, "content", "toolCalls");
        if (StringUtils.isNotBlank(message.getContent())) {
            chatMessage.setContent(Content.ofText(message.getContent()));
        }
        if (ObjectUtils.isNotEmpty(message.getContents())) {
        ...
        if (ObjectUtils.isNotEmpty(message.getToolCalls())) {
    ...
    public static Message convert(ChatMessage message) {
        Message target = BeanCopy.copy(message, Message.class, "content", "toolCalls");
        if (message.getContent() != null) {
            if (StringUtils.isNotBlank(message.getContent().getText())) {
                target.setContent(message.getContent().getText());
            }
            if (ObjectUtils.isNotEmpty(message.getContent().getMultiModals())) {
